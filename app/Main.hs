{-# OPTIONS_GHC -fno-warn-missing-methods #-}       -- turn off warning of not meeting minimal complete definintion of class
{-# LANGUAGE MultiParamTypeClasses #-}              -- type classes which can take multiple arguments
{-# LANGUAGE FunctionalDependencies #-}             -- constrains the parameters of type classes fundep
{-# LANGUAGE FlexibleInstances #-}                  -- allows the ehad of the instance declaration to mention arbitrary nested types
{-# LANGUAGE UndecidableInstances #-}               -- permit definition of instances with nontermination
...
-- example of flexible instance: First takes list and returns x. Two possible instances determined by pattern matching
class First list x | list -> x
instance First Nil Nil
instance First (Cons x more) x

-- concatenate two lists together: using recursion ListConcat is called 
class ListConcat a b c | a b -> c
instance ListConcat Nil x x
instance (ListConcat as bs cs)      -- constraint
...
-- Natural Numbers by hand for positions on board
data Z
data S n
...
-- integers from 0 to 0
class Range n xs | n -> xs
instance Range Z Nil
instance (Range n xs)
    => Range (S n) (Cons n xs)

-- test cases for story completeness
